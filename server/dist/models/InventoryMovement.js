...
exports.MOVEMENT_TYPES = void 0;
const mongoose_1 = __importStar(require("mongoose"));
// ─── Movement Type Enum ──────────────────────────────────────
exports.MOVEMENT_TYPES = ['RECEIPT', 'DELIVERY', 'TRANSFER', 'ADJUSTMENT'];
// ─── Schema ──────────────────────────────────────────────────
...
inventoryMovementSchema.pre('validate', function () {
    const movementType = this.get('type');
    ...
    switch (movementType) {
        case 'RECEIPT':
            if (!to)
                throw new Error('RECEIPT requires a toLocation');
            break;
        case 'DELIVERY':
            if (!from)
                throw new Error('DELIVERY requires a fromLocation');
            break;
        case 'TRANSFER':
            if (!from || !to)
        ...
        case 'ADJUSTMENT':
            if (!from)
                throw new Error('ADJUSTMENT requires a fromLocation');
            break;
    }
});
// ─── Serialization ──────────────────────────────────────────
